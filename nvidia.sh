...
if [ "$ONLY_CONFIG" = false ]; then
    ...
    if lspci | grep -i nvidia > /dev/null; then
        ...
        # Install NVIDIA Prime for hybrid graphics (laptops)
        read -p "Is this a laptop with hybrid graphics? (y/n) " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            msg "Installing NVIDIA Prime..."
            sudo apt-get install -y nvidia-prime
            
            # Create prime-switch script
            mkdir -p "$CONFIG_DIR/scripts"
            cat > "$CONFIG_DIR/scripts/prime-switch" << 'EOF'
#!/bin/bash
# NVIDIA Prime switcher
case "$1" in
    "intel")
        sudo prime-select intel
        echo "Switched to Intel graphics. Reboot required."
        ;;
    "nvidia")
    ...
    "query")
        ...
        ;;
    *)
        echo "Usage: prime-switch [intel|nvidia|query]"
        ;;
esac
EOF
            chmod +x "$CONFIG_DIR/scripts/prime-switch"
        fi
...
EOF
        ...
        msg "${YELLOW}No NVIDIA GPU detected. Skipping NVIDIA driver installation.${NC}"
    fi
else
...
cp -r "$SCRIPT_DIR"/i3/* "$CONFIG_DIR"/ 2>/dev/null || msg "Note: i3 config directory not found or empty"
...
EOF
    ...
    cp -r "$SCRIPT_DIR"/i3/* "$CONFIG_DIR"/ 2>/dev/null || true
...
if lspci | grep -i nvidia > /dev/null 2>&1; then
    ...
    echo -e "  • ${YELLOW}prime-switch${NC} - Switch between Intel/NVIDIA (laptops)"
    echo -e "  • ${YELLOW}nvtop${NC} - GPU monitoring (like htop for GPU)"
    echo -e "  • ${YELLOW}check-gpu${NC} - Check GPU acceleration status"
    echo -e "\n${YELLOW}REQUIRED: Reboot your system to enable NVIDIA drivers!${NC}"
    echo -e "${CYAN}════════════════════════════════════════════════════════════${NC}\n"
fi
...
echo -e "   • If NVIDIA drivers were installed, ${YELLOW}REBOOT IS REQUIRED${NC}"
echo -e "   • After reboot, run: ${CYAN}nvidia-smi${NC} to verify installation"
echo -e "   • Check: ${CYAN}glxinfo | grep 'OpenGL renderer'${NC} for acceleration"
echo ""
